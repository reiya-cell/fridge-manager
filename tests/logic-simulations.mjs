...
// 13: いらすとや画像・出典・ローカルキャッシュ。
check(source.includes("assets/irasutoya/meat.png"),'肉カテゴリ画像がありません');
check(source.includes("assets/irasutoya/fish.jpg"),'魚カテゴリ画像がありません');
check(source.includes('<img class="category-pictogram"'),'カテゴリ画像を描画していません');
check(source.includes('<img class="food-pictogram"'),'食材ボタンにカテゴリ画像を再利用していません');
check(html.includes('イラスト：'),'画面内の出典表示がありません');
