...
check(html.includes('styles.css?v=15')&&html.includes('app.js?v=19'),'CSS/JSのキャッシュ更新番号がありません');
...
check(html.includes('app.js?v=19'),'JavaScriptのキャッシュ更新番号がありません');
...
// 19: ユーザー追加食材だけを確認後に削除し、端末保存へ反映。
check(source.includes('function customFoodIcon(name)'),'追加食材専用の表示がありません');
check(source.includes('function escapeHtml(value)'),'追加食材名の安全な表示処理がありません');
check(source.includes('data-delete-custom'),'追加食材の削除ボタンがありません');
check(source.includes('このカテゴリから削除しますか？'),'削除前の確認がありません');
...
check(html.includes('app.js?v=19'),'JavaScriptのキャッシュ更新番号がありません');
results.push('19. 追加食材限定削除・確認・端末保存: PASS');
