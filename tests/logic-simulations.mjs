...
check(html.includes('styles.css?v=30')&&html.includes('app.js?v=30'),'CSS/JSのキャッシュ更新番号がありません');
...
check(html.includes('app.js?v=30'),'JavaScriptのキャッシュ更新番号がありません');
...
check(html.includes('styles.css?v=30'),'CSSのキャッシュ更新番号がありません');
...
check(html.indexOf("document.getElementById('itemCount')")<html.indexOf('app.js?v=30'),'件数先行表示がapp.jsより後です');
...
check(html.includes('cloud-config.js?v=30')&&source.includes('@supabase/supabase-js@2'),'Supabaseクライアント設定がありません');
