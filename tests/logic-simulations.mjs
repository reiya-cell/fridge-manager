...
check(html.includes('styles.css?v=39')&&html.includes('app.js?v=39'),'CSS/JSのキャッシュ更新番号がありません');
...
check(html.includes('app.js?v=39'),'JavaScriptのキャッシュ更新番号がありません');
...
check(html.includes('styles.css?v=39'),'CSSのキャッシュ更新番号がありません');
...
check(html.indexOf("document.getElementById('itemCount')")<html.indexOf('app.js?v=39'),'件数先行表示がapp.jsより後です');
...
check(html.includes('cloud-config.js?v=39')&&source.includes('@supabase/supabase-js@2'),'Supabaseクライアント設定がありません');
...
check(html.includes('app.js?v=39')&&serviceWorker.includes("fridge-manager-v39"),'公開キャッシュ番号が一致しません');
