...
vm.runInContext(`${definitions};globalThis.testApi={normalizeFoodText,presetFor,ruleFor,itemLiters,roomUsage,roomCapacity,freshnessFor,FRIDGE};`,context);
...
check(html.includes('styles.css?v=43')&&html.includes('app.js?v=43'),'CSS/JSのキャッシュ更新番号がありません');
...
check(html.includes('app.js?v=43'),'JavaScriptのキャッシュ更新番号がありません');
...
check(html.includes('styles.css?v=43'),'CSSのキャッシュ更新番号がありません');
...
check(html.indexOf("document.getElementById('itemCount')")<html.indexOf('app.js?v=43'),'件数先行表示がapp.jsより後です');
...
check(html.includes('cloud-config.js?v=43')&&source.includes('@supabase/supabase-js@2'),'Supabaseクライアント設定がありません');
...
check(html.includes('app.js?v=43')&&serviceWorker.includes("fridge-manager-v43"),'公開キャッシュ番号が一致しません');
