...
vm.runInContext(`${definitions};globalThis.testApi={normalizeFoodText,presetFor,ruleFor,itemLiters,roomUsage,roomCapacity,remaining,freshnessFor,FRIDGE};`,context);
...
check(html.includes('styles.css?v=47')&&html.includes('app.js?v=47'),'CSS/JSのキャッシュ更新番号がありません');
...
check(html.includes('app.js?v=47'),'JavaScriptのキャッシュ更新番号がありません');
...
check(html.includes('styles.css?v=47'),'CSSのキャッシュ更新番号がありません');
...
check(html.indexOf("document.getElementById('itemCount')")<html.indexOf('app.js?v=47'),'件数先行表示がapp.jsより後です');
...
check(html.includes('cloud-config.js?v=47')&&source.includes('@supabase/supabase-js@2'),'Supabaseクライアント設定がありません');
...
check(html.includes('js/app-utils.js?v=47')&&html.includes('js/camera-service.js?v=47')&&html.includes('app.js?v=47')&&serviceWorker.includes("fridge-manager-v47"),'公開キャッシュ番号が一致しません');
...
check(html.includes('id="confirmItemDialog"')&&html.includes('id="confirmItemName"'),'登録確認ダイアログがありません');
check(source.includes('function buildItemDraft')&&source.includes('function showItemConfirmation'),'登録下書きまたは確認表示がありません');
check(source.includes("$('#changeConfirmedItem').onclick=returnToItemForm"),'変更する操作がありません');
check(source.includes("$('#saveConfirmedItem').onclick")&&source.includes('items.unshift(pendingDraft)'),'確認後の確定保存がありません');
check(!source.includes("$('#itemForm').addEventListener('submit'"),'旧バーコード送信イベントが残っています');
results.push('32. 登録内容1画面確認・変更復帰・確定保存・バーコード統一: PASS');
...
check(html.indexOf('js/app-utils.js?v=47')<html.indexOf('js/camera-service.js?v=47')&&html.indexOf('js/camera-service.js?v=47')<html.indexOf('app.js?v=47'),'共通処理またはカメラ処理の読込順が不正です');
