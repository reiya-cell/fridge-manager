...
const utilsSource=await readFile(new URL('../js/app-utils.js',import.meta.url),'utf8');
const cameraSource=await readFile(new URL('../js/camera-service.js',import.meta.url),'utf8');
...
vm.runInContext(`${definitions};globalThis.testApi={normalizeFoodText,presetFor,ruleFor,itemLiters,roomUsage,roomCapacity,remaining,freshnessFor,needsStorageChoice,storageForName,autoLocationFor,FRIDGE};`,context);
...
check(source.includes("kind=pendingExpiryKind||$('#itemExpiryKind').value")&&source.includes('expiryKind:kind'),'期限種別が確認下書きへ保存されません');
...
check(cameraSource.includes("drawImage(video, 0, 0)"),'映像からOCR画像を生成していません');
...
check(source.includes("setTimeout(()=>$('#expiryDate').focus()"),'食材選択後に期限欄へ進みません');
...
check(html.includes('styles.css?v=51')&&html.includes('app.js?v=51'),'CSS/JSのキャッシュ更新番号がありません');
...
check(html.includes('app.js?v=51'),'JavaScriptのキャッシュ更新番号がありません');
...
check(html.includes('styles.css?v=51'),'CSSのキャッシュ更新番号がありません');
...
check(html.includes('class="quick-expiry-fields"')&&html.includes('<input id="expiryDate" type="date" required><button type="button" id="manualExpiryButton"'),'期限確認欄が主画面にありません');
check(html.includes('styles.css?v=51'),'CSSのキャッシュ更新番号がありません');
results.push('22. スマホ横1行バーコード・期限欄直下カメラ読取: PASS');

// 23: 在庫カードを情報2行へまとめて縦余白を削減。
check(source.includes('class="item-card compact-item-card freshness-${freshness.tone}"'),'鮮度付きコンパクト在庫カードがありません');
...
check(html.indexOf("document.getElementById('itemCount')")<html.indexOf('app.js?v=51'),'件数先行表示がapp.jsより後です');
check(utilsSource.includes('function readStoredItems(fallback = [])'),'例外安全な在庫読込がありません');
...
check(html.includes('cloud-config.js?v=51')&&source.includes('@supabase/supabase-js@2'),'Supabaseクライアント設定がありません');
...
check(source.includes("id:'other',name:'その他',mark:'その他'"),'その他カテゴリー定義がありません');
check(source.includes("'その他':'other'"),'その他カテゴリーの連携先が不正です');
check(html.includes('data-quick-category=\"other\"'),'JavaScript前のその他表示がありません');
check(html.includes('js/app-utils.js?v=51')&&html.includes('js/camera-service.js?v=51')&&html.includes('app.js?v=51')&&serviceWorker.includes("fridge-manager-v51"),'公開キャッシュ番号が一致しません');
results.push('29. その他カテゴリー・初期表示・双方向連携・キャッシュ更新: PASS');

check(html.includes('id="todayView"')&&html.includes('data-view="today"'),'今日使う画面がありません');
check(html.includes('id="openState"')&&html.includes('id="itemExpiryKind"'),'開封状態または期限種別がありません');
check(source.includes('function todayCard')&&source.includes('data-consume="half"'),'消費操作がありません');
check(source.includes('function consumeItem')&&source.includes("action==='freeze'"),'在庫減算または冷凍操作がありません');
check(html.includes('id="undoBar"')&&source.includes("$('#undoAction').onclick"),'取り消し操作がありません');
results.push('30. 今日使う・開封状態・期限根拠・消費操作・取り消し: PASS');

const navOrder=['data-view="add"','data-view="inventory"','data-view="alerts"','data-view="today"','data-view="foodSettings"','data-view="fridgeSettings"'].map(token=>html.indexOf(token));
check(navOrder.every((value,index)=>value>=0&&(index===0||value>navOrder[index-1])),'タブバーの順番が不正です');
check(html.includes('grid-template-columns:repeat(3,minmax(0,1fr))')&&html.includes('position:fixed;z-index:80'),'スマホ下部3列タブバーがありません');
results.push('31. 下部タブバー3列2段・指定順序・固定配置: PASS');

check(source.includes('function needsStorageChoice')&&source.includes("['肉類','魚介類'].includes(category)"),'肉・魚だけ保存状態を選ぶ判定がありません');
check(api.needsStorageChoice('肉類')&&api.needsStorageChoice('魚介類')&&!api.needsStorageChoice('野菜'),'保存状態選択の対象カテゴリが不正です');
check(source.includes('function autoLocationFor')&&source.includes("category==='野菜'&&vegetable"),'野菜室への自動配置がありません');
check(api.autoLocationFor('冷蔵','野菜')==='vegetable','野菜が野菜室へ自動配置されません');
check(api.autoLocationFor('冷凍','肉類')!=='ice','冷凍食材が製氷室へ自動配置されます');
check(api.ruleFor('冷凍食品').category==='冷凍食品'&&api.storageForName('冷凍食品')==='冷凍','冷凍食品が冷凍保存になりません');
check(api.autoLocationFor('冷凍','冷凍食品')==='freezer','冷凍食品が冷凍室へ自動配置されません');
check(html.includes('id="detailSettings"')&&html.indexOf('id="expiryDate"')<html.indexOf('id="detailSettings"')&&html.indexOf('id="quantity"')>html.indexOf('id="detailSettings"'),'数量や詳細項目が詳細設定へ移動していません');
check(source.includes('function completeItemRegistration')&&source.includes("if(currentStep===1&&needsStorageChoice")&&source.includes('completeItemRegistration(buildItemDraft())'),'通常食材の即登録または肉魚の保存状態分岐がありません');
check(!source.includes("$('#itemForm').addEventListener('submit'"),'旧バーコード送信イベントが残っています');
results.push('32. 期限中心登録・詳細設定退避・自動配置・確認なし登録: PASS');

check(api.freshnessFor(4).tone==='blue'&&api.freshnessFor(4).label==='余裕あり','4日以上の青判定が不正です');
check(api.freshnessFor(2).tone==='yellow'&&api.freshnessFor(2).status==='あと2日','1～3日の黄判定または残日数が不正です');
check(api.freshnessFor(0).tone==='red'&&api.freshnessFor(0).status==='今日まで','当日の赤判定が不正です');
check(api.freshnessFor(-2).tone==='red'&&api.freshnessFor(-2).status==='2日超過','期限超過の赤判定が不正です');
check(html.includes('.freshness-blue')&&html.includes('.freshness-yellow')&&html.includes('.freshness-red'),'鮮度3色のCSSがありません');
...
check(api.remaining({expiry:todayIso})===0,'期限当日が0日として計算されません');
results.push('33. 鮮度信号色・状態文言・残日数の共通判定: PASS');

// 34: 共通処理を分離し、読込順とオフラインキャッシュを維持。
check(utilsSource.includes('global.FridgeUtils'),'共通処理モジュールが公開されていません');
check(appSource.includes('=globalThis.FridgeUtils'),'app.jsが共通処理を利用していません');
check(html.indexOf('js/app-utils.js?v=51')<html.indexOf('js/camera-service.js?v=51')&&html.indexOf('js/camera-service.js?v=51')<html.indexOf('app.js?v=51'),'共通処理またはカメラ処理の読込順が不正です');
check(serviceWorker.includes("'./js/app-utils.js'"),'共通処理がオフラインキャッシュにありません');
results.push('34. 共通処理分離・読込順・オフラインキャッシュ: PASS');

// 35: カメラのライフサイクルとフレーム取得を専用ファイルへ分離。
check(cameraSource.includes('global.FridgeCamera'),'カメラサービスが公開されていません');
check(cameraSource.includes('function stopBarcode')&&cameraSource.includes('function stopExpiry'),'カメラ停止処理が分離されていません');
check(cameraSource.includes('async function startBarcode')&&cameraSource.includes('async function startExpiry'),'カメラ開始処理が分離されていません');
check(cameraSource.includes('function captureFrame'),'期限画像のフレーム取得が分離されていません');
check(!appSource.includes('let barcodeStream=')&&!appSource.includes('let expiryStream='),'app.jsにカメラストリーム状態が残っています');
check(serviceWorker.includes("'./js/camera-service.js'"),'カメラサービスがオフラインキャッシュにありません');
results.push('35. カメラ開始・停止・検出・フレーム取得の分離: PASS');
