...
vm.runInContext(`${definitions};globalThis.testApi={normalizeFoodText,presetFor,ruleFor,itemLiters,roomUsage,roomCapacity,remaining,freshnessFor,needsStorageChoice,autoLocationFor,FRIDGE};`,context);
...
check(html.includes('styles.css?v=49')&&html.includes('app.js?v=49'),'CSS/JSのキャッシュ更新番号がありません');
...
check(html.includes('app.js?v=49'),'JavaScriptのキャッシュ更新番号がありません');
...
check(html.includes('styles.css?v=49'),'CSSのキャッシュ更新番号がありません');
...
check(html.indexOf("document.getElementById('itemCount')")<html.indexOf('app.js?v=49'),'件数先行表示がapp.jsより後です');
...
check(html.includes('cloud-config.js?v=49')&&source.includes('@supabase/supabase-js@2'),'Supabaseクライアント設定がありません');
...
check(html.includes('js/app-utils.js?v=49')&&html.includes('js/camera-service.js?v=49')&&html.includes('app.js?v=49')&&serviceWorker.includes("fridge-manager-v49"),'公開キャッシュ番号が一致しません');
...
check(html.indexOf('js/app-utils.js?v=49')<html.indexOf('js/camera-service.js?v=49')&&html.indexOf('js/camera-service.js?v=49')<html.indexOf('app.js?v=49'),'共通処理またはカメラ処理の読込順が不正です');
