...
check(html.includes('styles.css?v=50')&&html.includes('app.js?v=50'),'CSS/JSのキャッシュ更新番号がありません');
...
check(html.includes('app.js?v=50'),'JavaScriptのキャッシュ更新番号がありません');
...
check(html.includes('styles.css?v=50'),'CSSのキャッシュ更新番号がありません');
...
check(html.indexOf("document.getElementById('itemCount')")<html.indexOf('app.js?v=50'),'件数先行表示がapp.jsより後です');
...
check(html.includes('cloud-config.js?v=50')&&source.includes('@supabase/supabase-js@2'),'Supabaseクライアント設定がありません');
...
check(html.includes('js/app-utils.js?v=50')&&html.includes('js/camera-service.js?v=50')&&html.includes('app.js?v=50')&&serviceWorker.includes("fridge-manager-v50"),'公開キャッシュ番号が一致しません');
...
check(source.includes('function completeItemRegistration')&&source.includes("$('#itemForm').onsubmit=e=>{e.preventDefault();completeItemRegistration(buildItemDraft())"),'確認画面を省いた即登録になっていません');
check(!source.includes("$('#itemForm').addEventListener('submit'"),'旧バーコード送信イベントが残っています');
results.push('32. 肉魚のみ保存状態選択・自動配置・確認なし登録: PASS');
...
check(html.indexOf('js/app-utils.js?v=50')<html.indexOf('js/camera-service.js?v=50')&&html.indexOf('js/camera-service.js?v=50')<html.indexOf('app.js?v=50'),'共通処理またはカメラ処理の読込順が不正です');
